...
public class GaussianRefiner implements SubPixelRefiner, OptimizableFunction {
	...
	int x0_,y0_;
	ImageProcessor ip_;
	FunctionOptimizer fo_;
	double [] parameters_; 

	public GaussianRefiner(ImageProcessor ip){
		ip_ = ip;
		fo_ = new FunctionOptimizer();
	...
	public int refine(double x, double y) {
		...
		fo_.setDimension(4);
		fo_.setInitialX(parameters_);
		fo_.setOptimizationMode(FunctionOptimizer.OptimizationMode.Function);
		fo_.optimizeFunction(this);
		
		if (fo_.getTerminationCode() == FunctionOptimizer.TerminationCode.IterationLimit) {
			//IJ.log("iterationlimit");
			return -1;
		}
		if (fo_.getTerminationCode() == FunctionOptimizer.TerminationCode.TooManyLargeStepsProbableUnboundFunction) {
			//IJ.log("unbound");
			return -2;
		}

		parameters_ = fo_.getOptimum();
		return 0;
	}
	...
	public double getQuality() {
		return fo_.getFunctionAtOptimum();
	}
